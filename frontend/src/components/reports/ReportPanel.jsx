...
import { generateReport } from '../../services/segmentationService';
import { FiFileText, FiRefreshCcw, FiChevronDown, FiChevronUp, FiDownload } from 'react-icons/fi';
...
const ReportPanel = ({ scanData, userRole, scanId }) => {
  ...
  const handleGenerate = async () => {
    try {
      setLoading(true);
      const res = await generateReport(scanId);
      if (res.success) {
        setReports(res.data);
      }
    } catch (err) {
  ...
  if (!scanData) return (
     <div className="h-full flex items-center justify-center p-8 text-center text-slate-500">
        <p>Select a scan to view data and AI insights</p>
     </div>
  );

  const { segmentationData } = scanData;
        ...
        {reports ? (
            <div className="flex flex-col h-full overflow-hidden">
             ...
             <div className="flex-1 flex flex-col items-center justify-center text-center p-6 bg-slate-800 rounded-xl border border-slate-700">
                  ...
                  <h4 className="text-lg font-medium mb-2">Generate Reports</h4>
                  <p className="text-sm text-slate-400 mb-6">Create AI-powered clinical and patient-friendly reports based on segmentation results.</p>
                  
                  <button 
                     onClick={handleGenerate} 
                     disabled={loading || userRole !== 'doctor'} // Only doctors generate reports
                  ...
                  >
                      {loading ? (
                          <><FiRefreshCcw className="animate-spin mr-2" /> Generating...</>
                      ) : (
                          <>Generate with Gemini AI</>
                      )}
                  </button>
                  {userRole !== 'doctor' && <p className="text-xs text-red-400 mt-2">Only Doctors can generate new reports.</p>}
             </div>
        )}
      </div>
    </div>
